...
function Product() {
  const {data: products, isLoading: productLoading, error, refetch} = useGetAllProductQuery(4);
        ...
        <div className='w-full flex flex-col items-center justify-center'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full'>
                    ...
                    {(products?.data || []).map((product) => (
                      <div key={product._id} className='bg-gradient-to-t from-[#BEF6A5] to-[#A8E367] rounded-md w-full p-4'>
                        <div className='flex items-center justify-center'>
                          <img src={product.image} className='w-52 h-52 object-cover' alt="" />
                        </div>
