  ...
  return (
    <div className='flex items-start flex-col lg:flex-row'>
      <div className='bg-[#1B57C7] h-auto lg:h-screen w-full lg:w-1/4 flex flex-col gap-8 items-start py-32 px-4'>
        <h1 className='text-white text-sm lg:text-lg cursor-pointer' onClick={() => setWord("shops")}>Shop Detail</h1>
        <h1 className='text-white text-sm lg:text-lg cursor-pointer' onClick={() => setWord("shop-foods")}>Shop Foods</h1>
      ...
      <div className='w-full lg:w-3/4 py-1 lg:py-2'>
        <div className='flex items-center justify-between px-4'>
          ...
          <div>
            {word === "add-shop" ? (
              <button className='border bg-blue-500 text-white rounded-sm px-4 py-1 lg:py-2' onClick={() => setWord("shops")}>Back</button>
            ) : word === "shops" ? (
             <button className={`border bg-blue-500 text-white rounded-sm px-4 py-1 lg:py-2`} onClick={() => setWord("add-shop")}>Add / Update Shop</button>
            ): (<></>)}
          </div>
        </div>
        ...
        {word === "add-shop" ? (
          <div className='p-4 h-auto overflow-y-auto'>
            <form className='grid gap-4 py-8 px-2' onSubmit={submitHandler}>
              ...
              <div className='grid grid-cols-1 lg:grid-cols-3 gap-2 lg:gap-1 items-center'>
                <label className='text-sm lg:text-lg col-span-1'>Add Shop Image</label>
          ...
          <div className='p-4 h-auto overflow-y-auto w-full'>
            <div className='flex flex-col sm:flex-row items-center justify-between'>
                ...
                <button className={`border bg-blue-500 w-full sm:w-auto text-white rounded-sm px-4 py-1 lg:py-2`} onClick={() => setWord("add-food")}>Add Foods</button>
            </div>
            <div className=''>
               <FoodCards />
            </div>
            ...
            <form className='grid gap-4 py-8 px-2' onSubmit={submitHandler}>
               <div className='flex flex-col sm:flex-row items-center justify-between'>
                ...
                <button className={`border bg-blue-500 w-full sm:w-auto text-white rounded-sm px-4 py-1 lg:py-2`} onClick={() => setWord("shop-foods")}>Back</button>
               </div>
              <div className='grid grid-cols-1 lg:grid-cols-3 gap-2 lg:gap-1 items-center'>
                <label className='text-sm lg:text-lg col-span-1'>Add Food Image</label>
                <input className='border pl-4 col-span-2 py-1 lg:py-2 rounded-sm w-full' type="text" />
              </div>
